...
function relativeLuminance(hex: string): number {
  const channels = [0, 1, 2].map((offset) => Number.parseInt(hex.slice(1 + offset * 2, 3 + offset * 2), 16) / 255);
...
export function issueLabelStyle(color: string | null | undefined): CSSProperties | undefined {
  ...
  const foreground = ["#ffffff", "#111827"].find(
    (candidate) => contrastRatio(relativeLuminance(candidate), backgroundLuminance) >= MIN_CONTRAST,
  );
