...
3. **Press `Shift+T`**, pick a transport on the Confirm screen (Tailscale Funnel vs Cloudflare Tunnel, cards show each one's readiness), and wait ~10 seconds for the tunnel to come up.
...
- The tunnel stays up as a background daemon after you close the TUI. Press `Shift+T` to reattach, `S` to stop, or run `aoe serve --stop`.
...
- **Started `aoe serve` from the CLI instead**: press `Shift+T` in the TUI; it attaches to the running daemon.
